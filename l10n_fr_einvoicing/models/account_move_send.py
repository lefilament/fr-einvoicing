...
class AccountMoveSend(models.AbstractModel):
    ...
    def _is_applicable_to_company(self, method, company):
        # EXTENDS 'account'
        if method == "fr_pa":
            return company._fr_ctc_is_vat_registered(raise_if_misconfigured=False)
        else:
    ...
    def _get_placeholder_mail_attachments_data(
        self, move, invoice_edi_format=None, extra_edis=None
    ):
        ...
        if move.company_id._fr_ctc_is_vat_registered(raise_if_misconfigured=False):
            odoo_invoice_format = (
                move.company_id.fr_ctc_send_invoice_format or "facturx"
            )
            filename = move._prepare_en16931_filename(odoo_invoice_format)
            results.append(
                {
    ...
    def _display_attachments_widget(self, edi_format, sending_methods):
        ...
        ) or edi_format in ["ubl_pdf", "cii_pdf"]
    ...
    def _get_ubl_available_attachments(
        ...
        attachment_ids = [
            values["id"]
            for values in mail_attachments_widget
            if values.get("manual") or values.get("mail_template_id")
        ]
        attachments = self.env["ir.attachment"].browse(attachment_ids)

        if invoice_edi_format not in ["ubl_pdf", "cii_pdf"]:
    ...
    def _hook_invoice_document_before_pdf_report_render(self, invoice, invoice_data):
        ...
        if invoice.company_id._fr_ctc_is_vat_registered(raise_if_misconfigured=False):
            invoice_format = invoice_data["invoice_edi_format"]
            xml_content = invoice.generate_en16931_xml(
                "factur-x", "extended", invoice_format
            )

            filename = move._prepare_en16931_filename(odoo_invoice_format)
        if invoice.with_context(
            sending_method=invoice_data["sending_methods"]
        )._need_ubl_cii_xml(invoice_data["invoice_edi_format"]):
    ...
    def _hook_invoice_document_after_pdf_report_render(self, invoice, invoice_data):
        ...
        if invoice_data.get("ubl_cii_xml_options", {}).get("ubl_cii_format") in (
            "facturx",
            "zugferd",
        ):
        ...
        if (
            (
                ...
                in ("facturx", "zugferd")
                or (
                    invoice.commercial_partner_id.country_code in ("FR", "DE")
    ...
    def _needs_ubl_postprocessing(self, invoice_data):
        return "ubl_cii_xml_options" in invoice_data and invoice_data[
            "ubl_cii_xml_options"
        ]["ubl_cii_format"] not in ("facturx", "zugferd")
    ...
    def _postprocess_invoice_ubl_xml(self, invoice, invoice_data):
        ...
        anchor_elements = (
            project_refs
            if project_refs
            else tree.xpath("//*[local-name()='AccountingSupplierParty']")
        )
        if not anchor_elements:
            return

        xmlns_move_type = (
            "Invoice" if invoice.move_type == "out_invoice" else "CreditNote"
        )
        ...
        doc_type_code_vals = edi_model._get_document_type_code_vals(
            invoice, invoice_data
        )
        if doc_type_code_vals["value"]:
            doc_type_code_attrs = " ".join(
                f'{name}="{value}"'
                for name, value in doc_type_code_vals["attrs"].items()
            )
